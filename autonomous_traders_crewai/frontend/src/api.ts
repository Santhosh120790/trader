// Client for the trading floor's read-only HTTP API. All paths are relative;
// in dev the Vite proxy forwards /api to the FastAPI backend, so the browser
// sees one origin.
...
export function getTraderLogs(name: string, lastN = 20): Promise<LogRow[]> {
