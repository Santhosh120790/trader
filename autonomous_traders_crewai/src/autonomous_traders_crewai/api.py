"""Read-only HTTP API over the trading floor, for the React dashboard.

Everything here only reads accounts.db; trading itself happens out of band
whenever main.run_trader() executes a crew round. Run it from the project
root so it shares the same accounts.db the MCP servers write to:

    uv run uvicorn autonomous_traders_crewai.api:app --port 8000 --reload
"""
...
from .domain.database import read_log
from .main import TRADER_NAMES

load_dotenv(override=True)

LASTNAMES = ["Patience", "Bold", "Systematic", "Crypto", "Prime"]
...
roster = [
    ...
    for name, lastname in zip(TRADER_NAMES, LASTNAMES)
]
roster_by_name = {trader["name"].lower(): trader for trader in roster}

app = FastAPI(title="Autonomous Traders")
...
def get_trader_logs(name: str, last_n: int = 20) -> list[dict]:
    """Recent account activity log lines, oldest first, with their panel colour."""
    require_trader(name)
